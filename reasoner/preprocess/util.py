...
def analyze_raw_splits(raw_splits: List[RawSplit], folder: str) -> RawDataset:
    ...
    for raw_split in raw_splits:
        ...
        with open(path.join(folder, raw_split.file), "r") as f:
            ...
            print("Creating IDs for relations...")
            for t in raw_split.data:
                if t[P] not in relation_map:
                    relation_map[t[P]] = len(relation_map)

            print("Creating IDs for entities...")
            for t in raw_split.data:
                if t[S] not in entity_map:
                    if t[S] in relation_map:
                        print(t[S], "in relation_map")
                        entity_map[t[S]] = relation_map[t[S]]
                    else:
                        entity_map[t[S]] = len(relation_map) + len(entity_map)
                if t[O] not in entity_map: 
                    if t[O] in relation_map:
                        print(t[O], "in relation_map")
                        print(relation_map[t[O]])
                        entity_map[t[O]] = relation_map[t[O]]
                    else:
                        entity_map[t[O]] = len(relation_map) + len(entity_map)
